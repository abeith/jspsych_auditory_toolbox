...
const info = <const>{
    ...
    parameters: {
        ...
        visual_times: {
            type: ParameterType.INT,
            pretty_name: "Visual stim presentation times",
            default: undefined,
            array: true
        }
    },
 ...
 * **av-audio-response**
 *
 * jsPsych plugin for playing and audio and visual stimuli then recordings an auditory response
 *
 * @author Alistair Beith
 * @see {@link https://github.com/abeith/jspsych_auditory_toolbox audio-audio-response plugin documentation at GitHub repo}
 */
class AVAudioResponsePlugin implements JsPsychPlugin<Info> {
    static info = info;
    private audio;
    private stream;
    ...
    trial(display_element: HTMLElement, trial: TrialType<Info>) {
        ...
        // Check that visual stim lengths match
        if(trial.visual_stim.length != trial.visual_times.length) stop();
        ...
        const draw = () => {
            ...
            ctx.fillStyle = (this.displayVal) ? "#fff" : "#000";
        ...
        const setupWorklets = async () => {
            ...
            await context.audioWorklet.addModule("assets/avWorkletProcessor.js");
            this.stimProcessor = new AudioWorkletNode(context, "av-worklet");
            this.micProcessor = new AudioWorkletNode(context, "av-worklet");
            ...
            this.stimProcessor.port.onmessage = (event) => {
                ...
                } else if (event.data.eventType === "trigger") {
                    this.displayVal = trial.visual_stim[event.data.trigger] == '1';
        ...
        const startTrial = async () => {
            ...
            for (let i = 0; i < trial.visual_times.length; i++) {
                this.stimProcessor.parameters.get("trigger").setValueAtTime(i, start_time + (trial.visual_times[i]/1000));
            };
...
export default AVAudioResponsePlugin;
